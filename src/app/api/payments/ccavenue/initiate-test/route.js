...
export async function POST(request) {
  try {
    // Unauthenticated test initiation for Rs. 1 (INR)
    ...
    const AMOUNT = typeof amount === 'string' && amount ? amount : '1.00';

    const orderId = `${Date.now()}${Math.floor(Math.random()*10000)}`; // numeric-only
    ...
    return NextResponse.json({ actionUrl, encRequest, accessCode: ACCESS_CODE, orderId, merchantId: MERCHANT_ID, amount: AMOUNT });
  } catch (e) {
    return NextResponse.json({ error: e?.message || 'Failed to initiate test payment' }, { status: 500 });
  }
}
